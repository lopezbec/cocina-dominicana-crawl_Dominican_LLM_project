...
class Crawler:
    def __init__(self, config: Any):
        ...
        self.domain_slug = self.config.domain_slug
    ...
    def scrape_with_retry(
        ...
        _max_retries = max_retries if max_retries is not None else self.config.crawler.get("max_retries", 3)
        _base_delay = base_delay if base_delay is not None else self.config.crawler.get("base_retry_delay", 2)
    ...
    def _is_valid_url(self, url: str) -> bool:
        ...
        # Check 2: Path must not contain the domain
        path = url.replace(self.base_url, "")
        if self.config.domain in path:
            return False

        return True

    def _extract_urls_from_markdown(self, markdown_content: str, source_url: str = "unknown") -> Set[str]:
                ...
                if not self._is_valid_url(url):
                    ...
                    with open(malformed_log, "a", encoding="utf-8") as f:
                        ...
                        if url_index != -1:
                            ...
                            f.write(f"CONTEXT: URL not found in markdown content (may have been constructed)\n")
    ...
    def _filter_valid_article_urls(self, urls: Set[str], section_url: str) -> List[str]:
        excluded_paths = self.config.get("excluded_article_paths", [])
        ...
        for url in urls:
            ...
            if url.endswith((".jpg", ".png", ".gif", ".jpeg")):
                ...
                should_exclude = True
            elif url_path in excluded_paths:
                should_exclude = True
    ...
    def discover_article_urls(self, section_url: str) -> List[str]:
        with PerformanceTimer() as timer:
            ...
            all_urls = self._extract_urls_from_markdown(markdown_content, source_url=section_url)
    ...
    def auto_discover_urls(self, page_url: str, use_config_filters: bool = True) -> List[str]:
        with PerformanceTimer() as timer:
            ...
            markdown_content = getattr(doc, "markdown")
            all_urls = self._extract_urls_from_markdown(markdown_content, source_url=page_url)
    ...
    def save_article(self, article_data: Dict, category: Optional[str] = None):
        ...
        with PerformanceTimer() as timer:
            ...
            content_filename = f"{doc_id}_{self.domain_slug}_{safe_filename}.md"
    ...
    def _process_article_batch(
        ...
        _skip_existing = skip_existing if skip_existing is not None else self.config.crawler.get("skip_existing", True)

        scraped_count = 0
        failed_count = 0
        skipped_count = 0

        delay = self.config.crawler.get("delay_seconds", 0.5)
    ...
    def scrape_section(self, section_key: str):
        with PerformanceTimer() as timer:
            ...
            metrics = self._process_article_batch(article_urls, category=section_info["name"])
    ...
    def crawl_category(
        ...
        _max_depth = max_depth if max_depth is not None else self.config.crawler.get("max_depth", 1)
        _skip_existing = skip_existing if skip_existing is not None else self.config.crawler.get("skip_existing", True)
        ...
        with LogContext.new_session(session_type="crawl_category", category_url=category_url):
            with PerformanceTimer() as timer:
                ...
                discovered_urls = self.auto_discover_urls(category_url, use_config_filters=True)
                    ...
                    for depth in range(2, _max_depth + 1):
                        ...
                        for url in discovered_urls:
                            if url not in visited_urls:
                                ...
                                deeper_urls = self.auto_discover_urls(url, use_config_filters=True)
    ...
    def _process_all_sections(self) -> Dict[str, int]:
        ...
        for section_key in self.sections:
            try:
                ...
                log_canonical(self.logger, "section_error", section_key=section_key, error=str(e))
                total_failed += 1
    ...
    def _finalize_scraping_session(self, session_info: Dict[str, Any], metrics: Dict[str, int]):
        ...
        log_canonical(self.logger, "summary_created", summary_file=str(summary_file))

    def scrape_all_sections(self):
        with LogContext.new_session(session_type="scrape_all_sections", domain=self.config.domain):
            if not self.sections:
                log_canonical(
                    self.logger,
                    "scrape_all_sections_skipped",
                    reason="no_sections_configured",
                    domain=self.config.domain,
                )
                print(f"\nNo sections configured for domain: {self.config.domain}")
                print("Use 'scrape', 'crawl', or 'discover' commands to scrape specific URLs")
                return
