...
from dominican_llm_scraper.core.processor.deduplication import stage_03_semantic
...
def _run_stage_01_and_02(base_dir: Path, near_threshold: float = 0.95) -> None:
...
def test_stage_03_is_deterministic_with_fixed_seed(tmp_path: Path) -> None:
    ...
    summary_1 = run_semantic_deduplication(
        tmp_path,
        embedding_provider=provider,
        min_token_count=10,
        chunk_token_count=100,
    ...
    first_rows = (tmp_path / "dedup_stage_03_semantic.jsonl").read_text(encoding="utf-8")

    summary_2 = run_semantic_deduplication(
        tmp_path,
        embedding_provider=provider,
        min_token_count=10,
        chunk_token_count=100,
    ...
    second_rows = (tmp_path / "dedup_stage_03_semantic.jsonl").read_text(encoding="utf-8")

    assert summary_1["duplicate_documents"] == summary_2["duplicate_documents"]
...
def test_stage_03_writes_eda_artifacts_and_schema(tmp_path: Path) -> None:
    docs = []
    embedding_map: Dict[str, List[float]] = {}
    for i in range(1, 6):
        doc_id = f"{i:04d}"
    ...
    assert (tmp_path / "dedup_stage_03_doc_metrics.parquet").exists()
    assert (tmp_path / "dedup_stage_03_cluster_metrics.parquet").exists()
    assert (tmp_path / "dedup_stage_03_pair_edges.parquet").exists()
    assert (tmp_path / "dedup_stage_03_run_summary.json").exists()
    assert (tmp_path / "dedup_stage_03_keep_manifest_eps_0p9.txt").exists()
    assert (tmp_path / "dedup_stage_03_keep_manifest_eps_0p95.txt").exists()

    doc_df = pd.read_parquet(tmp_path / "dedup_stage_03_doc_metrics.parquet")
    ...
    assert expected_doc_columns.issubset(set(doc_df.columns))
...
def test_stage_03_failfast_when_ollama_unavailable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="Ollama endpoint unavailable"):
        run_semantic_deduplication(tmp_path, failfast_ollama=True)
...
def test_stage_03_failfast_when_model_missing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="not available"):
        run_semantic_deduplication(tmp_path, failfast_model=True, model_name="qwen3-embedding:0.6b")
...
def test_stage_03_use_gpu_true_fails_without_gpu(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="GPU requested"):
        run_semantic_deduplication(tmp_path, embedding_provider=provider, use_gpu=True)
