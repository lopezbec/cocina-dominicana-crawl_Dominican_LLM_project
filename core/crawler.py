...
class Crawler:
    ...
    def scrape_with_retry(
        ...
        _max_retries = (
            ...
            else self.config.crawler.get("max_retries", 3)
        )
        _base_delay = (
            base_delay
            if base_delay is not None
            else self.config.crawler.get("base_retry_delay", 2)
        )
    ...
    def _extract_urls_from_markdown(self, markdown_content: str) -> Set[str]:
        ...
        if not patterns:
            domain_escaped = re.escape(urlparse(self.base_url).netloc)
            patterns = [
    ...
    def _filter_urls_by_config(self, urls: Set[str]) -> List[str]:
        filters = (
            self.config.filters.to_dict() if hasattr(self.config, "filters") else {}
        )
    ...
    def _process_article_batch(
        ...
        _skip_existing = (
            ...
            else self.config.crawler.get("skip_existing", True)
        )
    ...
    def crawl_category(
        ...
        _max_depth = (
            ...
            else self.config.crawler.get("max_depth", 1)
        )
        _skip_existing = (
            skip_existing
            if skip_existing is not None
    ...
    def scrape_all_sections(self):
        if not self.sections:
            ...
            print(
                "Use 'scrape', 'crawl', or 'discover' commands to scrape specific URLs"
            )
            return
