...
def scrape_list(args, crawler):
    ...
    log_canonical(
        logger, "cli_scrape_list_started", file=args.file, url_count=len(urls)
    )
...
def display_interactive_menu(
    urls: List[str], source_url: str, section_name: str
) -> int:
    print(f"\n{'=' * 70}")
    ...
    if args.interactive:
        ...
        if choice == 1:
            ...
            for i, url in enumerate(urls, 1):
                ...
                if result:
                    print(
                        f"  Success: {result['title']} ({result['word_count']} words)"
                    )
                    success_count += 1
                else:
                    print(f"  Failed to scrape")
            ...
            log_canonical(
                logger, "cli_discover_completed", url=args.url, urls_found=len(urls)
            )
            return 0
...
def process_to_plaintext(args, config):
    ...
    from core.config_loader import load_processing_patterns
    ...
    input_dir = (
        Path(args.input)
        if args.input
        else Path(config.get("output_dir", "scraped_content"))
    )
    output_dir = (
        Path(args.output)
        if args.output
        else Path(config.get("plaintext_output_dir", "scrapped_plain_text"))
    )
...
Examples:
  ...
  Process to plaintext:
    ...
    scrape_parser.add_argument(
        "--output", help="Output directory name (default: custom)"
    )
    scrape_parser.set_defaults(func=scrape_single_url)

    crawl_parser = subparsers.add_parser(
        "crawl", help="Crawl a category page and scrape discovered articles"
    )
    ...
    list_parser = subparsers.add_parser(
        "scrape-list", help="Scrape multiple URLs from a file"
    )
    ...
    all_parser = subparsers.add_parser(
        "scrape-all", help="Scrape all sections defined in site config"
    )
    all_parser.set_defaults(func=scrape_all_sections)

    discover_parser = subparsers.add_parser(
        "discover", help="Discover URLs from a page with interactive menu"
    )
    ...
    discover_parser.add_argument(
        "--no-filter", action="store_true", help="Disable config-based URL filtering"
    )
    discover_parser.add_argument(
        "--interactive",
        action="store_true",
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process scraped markdown to plain text"
    )
    process_parser.add_argument(
        "--input", help="Input directory (default: from config)"
    )
    process_parser.add_argument(
        "--output", help="Output directory (default: from config)"
    )
    ...
    try:
        ...
        print(f"Configuration: sites/{config.domain_slug}/config.yml\n")
