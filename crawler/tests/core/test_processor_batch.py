...
from dominican_llm_scraper.core.processor import batch
...
def test_process_all_files_removes_documents_with_50_or_fewer_characters(tmp_path, monkeypatch) -> None:
    ...
    config = SimpleNamespace(processing={"min_content_length": 50})

    batch.process_all_files(input_dir, output_dir, config)
